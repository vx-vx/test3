...
{
	std::string path = savepath + file;
	URLDownloadToFileA(NULL, url.c_str(), path.c_str(), 0, NULL);
}

static struct MemoryStruct LoadDll(std::filesystem::path DllPath) {
	struct MemoryStruct chunk {};
	chunk.memory = (char*)malloc(1);
	chunk.size = 0;

	std::fstream Library;
	Library.open(DllPath, std::ios_base::in | std::ios_base::binary);

	if (!Library.is_open()) {
		printf("[-] Can't open DLL to read\n");
		Library.clear();
		return chunk;
	}

	chunk.size = std::filesystem::file_size(DllPath);
	char* LibraryData = reinterpret_cast<char*>(new int[chunk.size]);
	Library.read(LibraryData, chunk.size);
	chunk.memory = LibraryData;
...
bool ManualMap(HANDLE hProc, std::filesystem::path DllPath) {
	...
	MemoryStruct Library = LoadDll(DllPath);

	BYTE* pSrcData = reinterpret_cast<BYTE*>(Library.memory);
